...
//		lcdltdc.hsw=8;				    //水平同步宽度
//		lcdltdc.vsw=4;				    //垂直同步宽度
//		lcdltdc.hbp=62;				    //水平后廊
//		lcdltdc.vbp=12;				    //垂直后廊
//		lcdltdc.hfp=66;				    //水平前廊
//		lcdltdc.vfp=20;				    //垂直前廊
		lcdltdc.pwidth=HORIZONTAL;			    //面板宽度,单位:像素
		lcdltdc.pheight=VERTICAL;		    //面板高度,单位:像素
		lcdltdc.hsw=HSPW;				    //水平同步宽度
		lcdltdc.vsw=VSPW;				    //垂直同步宽度
		lcdltdc.hbp=HBPD;				    //水平后廊
		lcdltdc.vbp=VBPD;				    //垂直后廊
		lcdltdc.hfp=HFPD;				    //水平前廊
		lcdltdc.vfp=VFPD;				    //垂直前廊

		LTDC_Clk_Set(360,4,RCC_PLLSAIDIVR_8);//设置像素时钟  45Mhz  
		//其他参数待定.
